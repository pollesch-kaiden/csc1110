...
import polleschk.BasicWord;
import polleschk.Bigram;
import polleschk.Driver;
import polleschk.VocabularyEntry;
import polleschk.Word;
    ...
    public void addVocabulary() {
        try {
            ...
            Assertions.assertEquals(secondThis,
                    ((VocabularyEntry) vocabulary.get(1)).getLocation().get(1));
            System.out.println(((VocabularyEntry)vocabulary.get(1)).getLocation());
        } catch (NoSuchMethodException e) {
            Assertions.fail("Missing class " + e.getMessage());
        } catch (IllegalAccessException | InvocationTargetException e) {
            System.err.println("Could not access the method " + e.getMessage());
